...
async function test() {
    ...
    const results = await computeRankingsOspreyWomens(sku);
    console.log("results", results);
}

async function computeRankingsOspreyMens(sku) {
    var products = null;
    try {
        products = await loadProductsData(
        	path.resolve(__dirname, BASE_URL)
        );
        return {
            weight: getStats(sku, fetchMensByWeightAsc(products)),
            volume: getStats(sku, fetchMensByVolumeDesc(products)),
        }
    } catch (err) {
...
async function computeRankingsOspreyWomens(sku) {
    var products = null;
    try {
        products = await loadProductsData(
        	path.resolve(__dirname, BASE_URL)
        );
        const packsByWeight = fetchWomensByWeightAsc(products);
        const packsByVolume = fetchWomensByVolumeDesc(products);
        return {
            weight: getStats(sku, packsByWeight),
            volume: getStats(sku, packsByVolume),
        }
    } catch (err) {
        console.log("Error reading products data", err);
    }
...
/************************************************
 * Commute Rankings / Stats
 ***********************************************/

function getStats(sku, sortedProducts) {
	const position = getPosition(sku, sortedProducts);
...
function fetchWomensByWeightAsc(products) {
    return fetchByWeight(products, 'female');
}
function fetchMensByWeightAsc(products) {
    return fetchByWeight(products, 'male');
}
function fetchByWeight(products, gender) {
    console.log(gender);
	const sql = `
		SELECT sku, name, gender, weight->data->0->raw as weight
...
function fetchWomensByVolumeDesc(products) {
    return fetchByVolume(products, 'female');
}
function fetchMensByVolumeDesc(products) {
    return fetchByVolume(products, 'male');
}
function fetchByVolume(products, gender) {
	const sql = `
		SELECT sku, name, gender, volume->data->0->raw as volume
		FROM ? WHERE name LIKE '%Osprey%' AND (gender = '${gender}' OR gender = 'unisex')
...
module.exports = {
    computeRankingsOspreyMens,
    computeRankingsOspreyWomens,
};
