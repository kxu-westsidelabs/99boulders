...
function compute(products) {
	const packs = fetchMensByWeightAsc(products);
	const sku = 134072; 	// osprey atmos
	const position = getPosition(sku, packs);
	const percentage = getPercentage(position, packs.length);

	console.log(sku, `${position} / ${packs.length}`, `top ${percentage}%`);
}
...
function fetchMensByWeightAsc(products) {
	const sql = `
		...
		FROM ? WHERE name LIKE '%Osprey%' AND (gender = 'male' OR gender = 'unisex')
